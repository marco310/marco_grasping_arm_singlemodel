...
const std::string SIM_TIMESTAMP_KEY = "sai2::grasp_marco::simulation::timestamp";

// object information initialization
Vector3d object_position = Vector3d::Zero();
Quaterniond object_orientation = Quaterniond::Identity();
...
int main() {
	...
	//q << -1.41691,-0.078084,2.28986,-2.83614,0.253998, 2.87176, 0.64619, -0.000841, 0.00338, -0.00255, -0.000142, -0.000831, 0.003291, -0.002455, -0.000392, -0.000819,  0.003197, -0.002404, -0.000304, -0.001929, 0.000258, 0.000275, 0.000764;
	//sim->setJointPositions(robot_name, q);
	robot->updateModel();
    ...
    GLFWwindow* window = glfwCreateWindow(windowW, windowH, "SAI2.0 - Grasp Marco", NULL, NULL);
...
void simulation(Sai2Model::Sai2Model* robot, Simulation::Sai2Simulation* sim)
{
	...
	robot->_q=Eigen::VectorXd::Zero(dof);
	robot->_dq=Eigen::VectorXd::Zero(dof);
	//robot->_q << -1.41691,-0.078084,2.28986,-2.83614,0.253998, 2.87176, 0.64619, -0.000841, 0.00338, -0.00255, -0.000142, -0.000831, 0.003291, -0.002455, -0.000392, -0.000819,  0.003197, -0.002404, -0.000304, -0.001929, 0.000258, 0.000275, 0.000764;
	robot->updateModel();
	...
	while (runloop) {
		...
		// sim->showContactInfo();
		sim->getContactList(contact_points, contact_forces, robot_name, "link_7.0");
